...
namespace JsonMenuBarDBApp.database_access_layer
{
    public class db
    {
        ...
        //Get Subtosubtosubcategory
        public DataSet get_SubtoSubtosubcategory(int subcatid)
        {
            SqlCommand com = new SqlCommand("Select * from tbl_subtosubtosubcategory where Subtosubcat_id=@subtosubcatid", con);
            com.Parameters.AddWithValue("@subtosubcatid", subcatid);
            SqlDataAdapter da = new SqlDataAdapter(com);
            DataSet ds = new DataSet();
            da.Fill(ds);
            return ds;
